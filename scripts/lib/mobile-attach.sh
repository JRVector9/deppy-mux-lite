...
cmux_attach_mac_bundle_id() {
  printf 'com.deppy-mux.app.debug.%s' "$(cmux_attach__bundle_seg "$1")"
}
...
# sanitized slug, matching reload.sh (`APP_NAME="deppy-mux-beta"`); the raw
# tag would miss for any tag whose slug differs (e.g. "Fix Foo" -> "fix-foo").
cmux_attach_mac_app_path() {
  local slug
  slug="$(cmux_attach__slug "$1")"
  printf '%s/Library/Developer/Xcode/DerivedData/cmux-%s/Build/Products/Debug/deppy-mux-beta.app' \
    "$HOME" "$slug"
}
...
cmux_attach_ensure_mac() {
  ...
  if [[ -S "$sock" ]]; then
    ...
    pkill -f "deppy-mux-beta.app/Contents/MacOS/deppy-mux-beta" 2>/dev/null || true
