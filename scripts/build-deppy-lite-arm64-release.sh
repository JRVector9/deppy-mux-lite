...
find_prebuilt_ghostty_helper() {
  ...
  for candidate in \
    "/Applications/deppy-mux-lite.app/Contents/Resources/bin/ghostty" \
...
# back to a prebuilt arm64 helper so local release builds still succeed. On CI
# (older macOS) the probe passes and the source build runs as usual. Disable
# with DEPPY_LITE_AUTO_GHOSTTY_HELPER=0.
if [[ -z "$PREBUILT_GHOSTTY_HELPER" \
  ...
  if [[ -n "$probe_zig" && -x "$probe_zig" ]] && ! zig_can_link_natively "$probe_zig"; then
    auto_helper="$(find_prebuilt_ghostty_helper arm64 || true)"
    if [[ -n "$auto_helper" ]]; then
      ...
      echo "warning: pinned Zig cannot build the Ghostty helper here and no prebuilt arm64 helper was found." >&2
