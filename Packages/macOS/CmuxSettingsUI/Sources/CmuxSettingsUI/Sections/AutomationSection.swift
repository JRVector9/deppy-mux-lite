...
public struct AutomationSection: View {
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: claudeCodeModel.current
                    ...
                    : String(localized: "settings.automation.claudeCode.subtitleOff", defaultValue: "Claude Code runs without deppy-mux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.claudeCode.note", defaultValue: "When enabled, deppy-mux wraps the claude command to inject session tracking and notification hooks. Disable if you prefer to manage Claude Code hooks yourself."))
        }
    }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: codexModel.current
                    ...
                    : String(localized: "settings.automation.codex.subtitleOff", defaultValue: "Codex runs without deppy-mux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.codex.note", defaultValue: "When enabled, deppy-mux wraps the codex command to inject session tracking and notification hooks. Disable if you prefer to manage Codex hooks yourself. deppy-mux still tracks live Codex sessions it can observe even when this is off. To also track Codex launched through a custom launcher that bypasses the wrapper (e.g. a subrouter), run `deppy-mux hooks setup --agent codex`, which installs hooks into ~/.codex/hooks.json."))
        }
    }
    ...
    private var autoNamingCard: some View {
        SettingsCard {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.workspaceAutoNaming.note", defaultValue: "When enabled, deppy-mux summarizes supported agent sessions into short workspace and tab names using each agent's own binary, refreshed as the topic shifts. Manual renames always win and stop auto-naming for that workspace or tab. Uses your agent account for the short summarization calls."))
            if autoNamingModel.current,
               !claudeCodeModel.current,
               autoNamingAgentModel.current == AutoNamingAgentCatalog.autoSlug || autoNamingAgentModel.current == "claude" {
                autoNamingFootnote(String(localized: "settings.automation.workspaceAutoNaming.hooksOffWarning", defaultValue: "Claude Code Integration is off, so Claude sessions will not be auto-named. Other supported agents still name when their deppy-mux hooks are installed."))
            }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: ampModel.current
                    ...
                    : String(localized: "settings.automation.amp.subtitleOff", defaultValue: "Amp runs without deppy-mux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.amp.note", defaultValue: "Hooks must be installed with `deppy-mux hooks amp install`. They no-op outside deppy-mux terminals. When disabled, the installed Amp plugin stays inactive without needing to be removed."))
        }
    }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: cursorModel.current
                    ...
                    : String(localized: "settings.automation.cursor.subtitleOff", defaultValue: "Cursor runs without deppy-mux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.cursor.note", defaultValue: "Hooks must be installed with `deppy-mux hooks cursor install`. They no-op outside deppy-mux terminals."))
        }
    }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: geminiModel.current
                    ...
                    : String(localized: "settings.automation.gemini.subtitleOff", defaultValue: "Gemini runs without deppy-mux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.gemini.note", defaultValue: "Hooks must be installed with `deppy-mux hooks gemini install`. They no-op outside deppy-mux terminals."))
        }
    }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: kiroModel.current
                    ...
                    : String(localized: "settings.automation.kiro.subtitleOff", defaultValue: "Kiro runs without deppy-mux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.kiro.note", defaultValue: "Hooks must be installed with `deppy-mux hooks kiro install`, then run Kiro with `kiro-cli chat --agent deppy-mux` (or set it as your default agent). They no-op outside deppy-mux terminals."))
        }
    }
