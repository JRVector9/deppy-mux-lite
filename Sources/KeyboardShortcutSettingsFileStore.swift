...
nonisolated private let cmuxSettingsFileStoreLogger = Logger(subsystem: "com.deppy-mux.app", category: "SettingsStore")
...
final class CmuxSettingsFileStore {
    ...
    static let schemaURLString = "https://raw.githubusercontent.com/JRVector9/deppy-mux-lite/main/web/data/cmux.schema.json"
    private static let legacySchemaURLString = "https://raw.githubusercontent.com/JRVector9/deppy-mux-lite/main/web/data/cmux-settings.schema.json"
    ...
    private func parseAppSection(
        ...
        if let value = jsonInt(section["closedItemHistoryCapacity"]) {
            if (1...10_000).contains(value) {
                snapshot.managedUserDefaults[SettingCatalog().app.closedItemHistoryCapacity.userDefaultsKey] = .int(value)
            } else {
                logInvalid("app.closedItemHistoryCapacity", sourcePath: sourcePath)
            }
        } else if section.keys.contains("closedItemHistoryCapacity") {
            logInvalid("app.closedItemHistoryCapacity", sourcePath: sourcePath)
        }
